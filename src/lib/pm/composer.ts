...
// state, the `actions` slot (Template, etc.) is. A staged attachment (a
// photo waiting in the composer) counts as content: the caption is optional.
export function canSend({
  ...
  return hasAttachment || value.trim().length > 0;
}
