    ...
    <div style={{ padding: "32px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "6px" }}>
            <h1 style={{ fontSize: "26px", fontWeight: 700, color: "#111827" }}>{campaign.name}</h1>
          ...
          <p style={{ fontSize: "14px", color: "#6b7280" }}>Sent {campaign.date}</p>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: "12px", marginBottom: "24px" }}>
        {[
          ...
          <div key={s.label} style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "10px", padding: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "24px" }}>
        ...
        <div style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "24px" }}>
          <h2 style={{ fontSize: "16px", fontWeight: 600, color: "#111827", marginBottom: "24px" }}>Performance Funnel</h2>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {funnelSteps.map((step, i) => (
              <div key={step.label}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
                  <span style={{ fontSize: "13px", color: "#4b5563" }}>{step.label}</span>
                  <span style={{ fontSize: "13px", fontWeight: 600, color: "#111827" }}>
                    {step.value.toLocaleString()} <span style={{ color: "#9ca3af", fontWeight: 400 }}>({step.pct}%)</span>
                  </span>
                </div>
                ...
                {i < funnelSteps.length - 1 && (
                  <div style={{ height: "8px", display: "flex", justifyContent: "center" }}>
                    <div style={{ width: "1px", height: "8px", backgroundColor: "#e5e7eb" }} />
                  </div>
                )}
              </div>
            ))}
        ...
        {/* Device Breakdown */}
        <div style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "24px" }}>
          <h2 style={{ fontSize: "16px", fontWeight: 600, color: "#111827", marginBottom: "24px" }}>Device Breakdown</h2>
            ...
            {devices.map((d) => (
              <div key={d.label}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
                  ...
                  <span style={{ fontSize: "14px", color: "#111827", flex: 1 }}>{d.label}</span>
      ...
      {/* Link Clicks */}
      <div style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb", borderRadius: "12px", padding: "24px" }}>
        <h2 style={{ fontSize: "16px", fontWeight: 600, color: "#111827", marginBottom: "20px" }}>Link Click Breakdown</h2>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              {["URL", "Total Clicks", "Unique Clicks", "Share"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "0 0 12px 0", fontSize: "11px", fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "1px solid #e5e7eb" }}>
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {links.map((l, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: i < links.length - 1 ? "1px solid #e5e7eb" : "none" }}>
                  <td style={{ padding: "14px 0", fontSize: "13px", color: "#B91C4A" }}>{l.url}</td>
                  <td style={{ padding: "14px 16px", fontSize: "14px", fontWeight: 600, color: "#111827" }}>{l.clicks}</td>
                  <td style={{ padding: "14px 16px", fontSize: "14px", color: "#4b5563" }}>{l.unique}</td>
                  <td style={{ padding: "14px 0", width: "200px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      ...
                      <span style={{ fontSize: "12px", color: "#6b7280", width: "32px" }}>{share}%</span>
                    </div>
                  </td>
                </tr>
              );
