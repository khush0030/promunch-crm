...
export async function fetchWaStanding(): Promise<WaStanding | null> {
  ...
  try {
    ...
    const body = await r.json().catch(() => null) as
      ...
      | null;
    let tier = body?.messaging_limit_tier ?? null;
    let quality = body?.quality_rating ?? null;

    // Phone-node fields are sometimes absent; the WABA phone_numbers edge
    // returns messaging_limit_tier reliably (see wa-meta-info).
    ...
    return { tier, quality, limit: tierDailyLimit(tier) };
  } catch {
    return null;
  }
}
